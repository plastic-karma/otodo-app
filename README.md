...
Tap **Search** in the workspace toolbar to reveal **Search all todos** and find any cached todo without changing the current filter, project, or Upcoming view. The search field stays hidden until opened; **Cancel** clears the search and hides it again. Search includes completed todos and matches every entered word across names, Markdown notes, project slugs, tags, links, and full IDs; clearing the field restores the previous view. It works entirely offline.
...
CI builds both Watch targets with the current SDK before creating and booting the iPhone/Watch simulator pair. The paired smoke check pins the installed iOS 18.6/watchOS 11.5 runtimes through `WATCH_SMOKE_IOS_VERSION` and `WATCH_SMOKE_WATCHOS_VERSION`; a missing requested runtime fails rather than silently changing coverage or downloading a platform. Cold 26.2 pairs have stalled in migration, installation, and WCSession transport on hosted runners. Both Apple test jobs use the native Arm64 `macos-15` runner; the paired check also records its CPU/memory allocation. It selects an available, shut-down, unpaired phone from the runner image, preferring a Pro model, or creates one if none is available. The Watch is always newly created. It verifies both devices belong to the active pair, activating it only when needed. Hosted iOS tests continue on the newest installed iOS runtime. Keeping compilation separate from simulator startup reduces memory pressure on hosted runners. A failed initial simulator data migration gets one bounded reboot with both boot logs retained; successful migration is still required before checking real live delivery and offline relaunch. After collecting evidence, an always-running cleanup step shuts down only the simulators assigned to that attempt. Watch-face placement and complication tap routing, large-file transfers, and expedited complication updates require a physical paired device check.

An activated Watch gets two minutes to finish its initial connection handshake before a bounded phone restart is considered. Recovery rechecks live readiness after collecting diagnostics and leaves reachable sessions, in-flight requests, and delivered snapshots running. If the pair still needs recovery, the Watch app pauses while its connectivity daemon remains running, then relaunches after the phone restores the same fixtures. Recovery never writes a Watch snapshot or suppresses an app error; real WCSession delivery and a subsequent offline cache load remain mandatory.
