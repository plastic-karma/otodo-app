...
Choose **… → Edit Project…** to change a project's name and Markdown notes. Changes save offline and update task-row and project-choice labels without renaming its slug or file, moving any todos, or restoring an archived project. Project slugs are generated only at creation. If the project changes after opening the editor, saving refuses the stale draft; close and reopen to load its current notes.
...
Type `#project-slug` in the name or notes to assign an existing project. Matching is whole-token and case-insensitive, with local autocomplete at the caret in either field. A completion replaces only that mention; surrounding prose, punctuation, and Markdown remain intact. Detected projects appear immediately and are included on Save alongside existing or explicitly selected projects. Unknown mentions, escaped markers, email addresses, and markers inside `https://`, `www.`, or protocol-relative URLs do not create assignments. Mentions remain in the saved text. Removing a mention does not silently remove an already-saved project assignment; remove that assignment under **Details** after editing the mention. **Save & Create Another** keeps the resulting project choices.
