...
CI builds both Watch targets before creating, explicitly activating, and booting a fresh iPhone/Watch simulator pair from the installed runtimes. Keeping compilation separate from simulator startup reduces memory pressure on hosted runners. The check verifies the active pair, successful boot migration, real live delivery, and offline relaunch. Watch-face placement and complication tap routing, large-file transfers, and expedited complication updates require a physical paired device check.
