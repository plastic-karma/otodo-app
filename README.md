...
An activated Watch that stays unreachable for 30 seconds without an outstanding request gets one bounded phone restart. This addresses cold pairs where watchOS reports a connected peer but never receives its first-unlock state. The Watch app pauses while its connectivity daemon remains running, then relaunches after the phone restores the same fixtures. Recovery retains diagnostics and never writes a Watch snapshot or suppresses an app error; real WCSession delivery and a subsequent offline cache load remain mandatory.
