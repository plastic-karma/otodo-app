...
/// Capture returns only after the normal durable Markdown workspace transaction succeeds.
enum SharedTaskCapture {
    ...
    static func attachmentContext() async throws -> AttachmentContext {
        ...
        guard let selection = try await selectionStore.load() else {
            throw OTodoError.validation(field: "workspace", message: "Open OTodo and choose a local or GitHub workspace before capturing todos.")
        }
    ...
    static func save(name: String, body: String, url: String? = nil, attachments: [AttachmentDraft] = [],
                     expectedSelection: WorkspaceSelection? = nil) async throws -> TodoTask {
